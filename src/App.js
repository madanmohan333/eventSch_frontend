...
function App() {
  const client_id =
    "154737886462-ef9cneipqh5p0j4pe561h1ofhmt1lpps.apps.googleusercontent.com";
  ...
  const handlebackfromevent = () => {
    ...
    if (showEventMeet === true) setShowEventMeet(false);
  };

  const handlebackfromMeet = () => {
    if (showMeets === true) setShowMeets(false);
  };
  ...
  const setShowAlert1 = (value) => setShowAlert(value);
  const setMessage1 = (value) => setMessage(value);
  const setType1 = (value) => setType(value);

  const [darkTheme, setDarkTheme] = useState("");
  const setDarkTheme1 = (value) => setDarkTheme(value);

  return (
    <EventState>
      {showAlert && <Alert message={message} type={type} />}

      {/* ✅ Google OAuth Provider (React 18 compatible) */}
